"""Sprint 17 customers + sales invoices core fields."""

from alembic import op
import sqlalchemy as sa


revision = "0017_customers_sales_invoices"
down_revision = "0016_5_stability_constraints"
branch_labels = None
depends_on = None
...
def upgrade() -> None:
    ...
    with op.batch_alter_table("sales_invoices") as batch:
        ...
        batch.create_index("ix_sales_invoices_tenant_customer", ["tenant_id", "customer_id"])
        batch.create_index("ix_sales_invoices_tenant_status", ["tenant_id", "status"])
        batch.create_foreign_key(
            "fk_sales_invoices_posting_journal_entry_id",
            "journal_entries",
        ...
        batch.create_foreign_key(
            "fk_sales_invoices_reversal_journal_entry_id",
            "journal_entries",
    ...
    op.execute("UPDATE sales_invoices SET subtotal = total_amount WHERE subtotal = 0")
    op.execute("UPDATE sales_invoices SET total = total_amount WHERE total = 0")

    with op.batch_alter_table("sales_invoice_lines") as batch:
        batch.create_index("ix_sales_invoice_lines_tenant_invoice", ["tenant_id", "invoice_id"])
        batch.create_unique_constraint(
            "uq_sales_invoice_lines_tenant_invoice_line_no",
...
def downgrade() -> None:
    with op.batch_alter_table("sales_invoice_lines") as batch:
        batch.drop_constraint("uq_sales_invoice_lines_tenant_invoice_line_no", type_="unique")
        batch.drop_index("ix_sales_invoice_lines_tenant_invoice")

    with op.batch_alter_table("sales_invoices") as batch:
        batch.drop_constraint("fk_sales_invoices_reversal_journal_entry_id", type_="foreignkey")
        batch.drop_constraint("fk_sales_invoices_posting_journal_entry_id", type_="foreignkey")
        batch.drop_index("ix_sales_invoices_tenant_status")
        batch.drop_index("ix_sales_invoices_tenant_customer")
