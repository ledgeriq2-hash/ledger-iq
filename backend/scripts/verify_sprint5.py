...
async def run() -> None:
    if not PERMISSION_CATALOG:
        raise VerificationError("permission catalog is empty")
    ...
    async with session_maker() as session:
        ...
        required_roles = ["ADMIN", "VIEWER", "OWNER", "ACCOUNTANT"]
        for role_name in required_roles:
            await _get_role(session, tenant_id, role_name)

        admin_role = await _get_role(session, tenant_id, "ADMIN")
        viewer_role = await _get_role(session, tenant_id, "VIEWER")

        admin_permissions = admin_role.permissions_json if isinstance(admin_role.permissions_json, dict) else {}
        if not admin_permissions:
            raise VerificationError("admin role missing permissions payload")
        ...
        posted_entry = await admin_service.post_entry(entry_id)
        posted_entry_id = posted_entry.id

        reversal = await admin_service.reverse_entry(posted_entry_id, reason="Sprint 5 verification")
        ...
        await _expect_app_exception(
            session,
            LedgerService(session=session, tenant_id=tenant_id, actor_id=viewer_user_id).post_entry(draft_entry_id),
            code="permission_denied",
            label="viewer post permission",
        )
        await _expect_app_exception(
            session,
            LedgerService(session=session, tenant_id=tenant_id, actor_id=viewer_user_id).reverse_entry(
                posted_entry_id,
                reason="viewer reversal attempt",
            ),
            code="permission_denied",
