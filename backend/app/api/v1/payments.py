...
from app.schemas.payment import PaymentCreate, PaymentList, PaymentPublic, PaymentUpdate
...
async def create_payment(
    ...
    return await payment_service.create_payment(session, tenant_id, payload)
...
async def delete_payment(
    ...
    deleted = await payment_service.delete_payment(session, tenant_id, payment_id)
    if not deleted:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Payment not found")
    return None
