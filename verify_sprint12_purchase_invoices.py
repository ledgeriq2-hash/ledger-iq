...
def _prepare_environment() -> None:
    ...
    if not loaded and not _load_env_fallback():
        print("verify_sprint12_purchase_invoices: env file not loaded; relying on process environment")
...
async def run() -> None:
    ...
    async with session_maker() as session:
        tenant = Tenant(
            name="Sprint 12 Purchase Invoices",
            slug=f"sprint12-purchase-{uuid.uuid4().hex[:8]}",
        )
        session.add(tenant)
    ...
    async with AsyncClient(transport=transport, base_url="http://test") as client:
        vendor = await _expect_status(
            ...
            json={"code": "VEND-PI-1", "name": "Sprint 12 Vendor"},
        )
        vendor_id = vendor.get("id")
        if not vendor_id:
            raise VerificationError("vendor id missing for purchase invoice test")

        invoice_no = f"PI-{uuid.uuid4().hex[:8]}"
    ...
    async with session_maker() as session:
        ...
        if entries:
            raise VerificationError("draft purchase invoice should not create journal entry")
    ...
    async with AsyncClient(transport=transport, base_url="http://test") as client:
        ...
        if not posted.get("posted_at"):
            raise VerificationError("posted_at missing after posting purchase invoice")
    ...
    async with session_maker() as session:
        ...
        if not lines:
            raise VerificationError("journal entry lines missing for purchase invoice")
        ...
        if debit_total != credit_total or debit_total != total_amount:
            raise VerificationError("purchase invoice journal entry not balanced")
        ...
        ap_credit = credit_by_account.get(ap_account_id)
        if ap_credit != total_amount:
            raise VerificationError("AP control credit line incorrect")
        unexpected_credits = [account_id for account_id in credit_by_account if account_id != ap_account_id]
        if unexpected_credits:
            raise VerificationError("unexpected credit accounts in purchase invoice entry")

        expected_debit_total = _quantize(line_one["amount"]) + _quantize(line_two["amount"])
        expense_debit = debit_by_account.get(expense_account_id, Decimal("0.00"))
        if expense_debit != expected_debit_total:
            raise VerificationError("expense debit total incorrect")
        unexpected_debits = [account_id for account_id in debit_by_account if account_id != expense_account_id]
        if unexpected_debits:
            raise VerificationError("unexpected debit accounts in purchase invoice entry")
    ...
    async with AsyncClient(transport=transport, base_url="http://test") as client:
        reversed_invoice = await _expect_status(
            ...
            200,
            headers=admin_headers,
            json={"reason": "Sprint 12 reversal"},
        )
    ...
    async with AsyncClient(transport=transport, base_url="http://test") as client:
        reversed_again = await _expect_status(
            ...
            200,
            headers=admin_headers,
            json={"reason": "Sprint 12 reversal again"},
        )
    ...
    async with AsyncClient(transport=transport, base_url="http://test") as client:
        lock_vendor = await _expect_status(
            ...
            201,
            headers=admin_headers,
            json={"code": "VEND-PI-LOCK", "name": "Locked Period Vendor"},
        )
        locked_vendor_id = lock_vendor.get("id")
        if not locked_vendor_id:
            raise VerificationError("locked period vendor id missing")
    ...
    print("Sprint 12 purchase invoices verification: OK")
...
def main() -> None:
    try:
        ...
        print(f"Sprint 12 purchase invoices verification: FAILED - {exc}")
        sys.exit(1)
