...
const SupplierPortal = () => {
  ...
  const { data: portalData, isLoading, error } = useQuery({
    queryKey: ["portal", "supplier", activeToken],
    queryFn: async () => {
      const [overview, orders, payments, settings] = await Promise.all([
        portalApi.supplierOverview(activeToken),
        portalApi.supplierOrders(activeToken),
        portalApi.supplierPayments(activeToken),
        portalApi.supplierSettings(activeToken),
      ]);
      return {
        ...overview,
        orders: orders?.orders || overview?.orders || [],
  ...
  const renderBody = () => {
    ...
    if (isLoading) {
      return <LoadingSpinner message="Loading supplier portal..." />;
    }
    if (error) {
  ...
  return (
    ...
    >
      <div style={{ maxWidth: "1080px", margin: "0 auto", display: "grid", gap: "1rem" }}>
        ...
        >
          <div>
            <h2 style={{ margin: 0, color: colors.text }}>Supplier Portal</h2>
          ...
          <nav style={{ display: "flex", gap: spacing.sm, flexWrap: "wrap" }}>
            ...
            <NavLink to={`${base}/orders`} style={({ isActive }) => navStyle(isActive)}>
              Orders
            </NavLink>
